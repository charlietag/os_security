...
sed -i 's/NGX_LOG_WARN/NGX_LOG_ERROR/g' ../modsecurity-nginx-${NGX_MOD_VER}/ngx_http_modsecurity_module.c  # make sure modsecurity writes log into nginx "error" log, instead of using nginx error_log path warn;
